...
import { getMataPelajaran, createMataPelajaran, updateMataPelajaran, deleteMataPelajaran, deleteAllMataPelajaran, logout } from "@/lib/api";
...
export default function MataPelajaranPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("data_mapel");
    if (saved) {
      const parsed = JSON.parse(saved);
      Promise.resolve().then(() => setMapel(parsed));
    } else {
      const initial: MataPelajaran[] = [
        { id_mapel: "M-001", kode_mapel: "MTK", nama_mapel: "Matematika" },
        { id_mapel: "M-002", kode_mapel: "IND", nama_mapel: "Bahasa Indonesia" },
        { id_mapel: "M-003", kode_mapel: "ING", nama_mapel: "Bahasa Inggris" },
      ];
      localStorage.setItem("data_mapel", JSON.stringify(initial));
  ...
  const handleOpenEdit = (m: MataPelajaran) => {
    setEditingMapel(m);
    setForm({ kode_mapel: m.kode_mapel, nama_mapel: m.nama_mapel });
    setFormError("");
    setShowModal(true);
  };

  const handleSave = async () => {
    if (!form.kode_mapel.trim() || !form.nama_mapel.trim()) {
      setFormError("Semua field harus diisi.");
      return;
    }
    if (form.kode_mapel.length > 6) {
      setFormError("Kode Mapel tidak boleh lebih dari 6 karakter.");
      return;
    }
    setIsSaving(true);
    setFormError("");
    ...
    try {
      if (editingMapel) {
        const res = await updateMataPelajaran(editingMapel.id_mapel, form);
        if (res && res.success === false) {
          setFormError(res.message || "Gagal memperbarui mata pelajaran.");
          setIsSaving(false);
          return;
        }
        const updated = mapel.map((m) =>
          m.id_mapel === editingMapel.id_mapel ? { ...m, ...form } : m
        );
        localStorage.setItem("data_mapel", JSON.stringify(updated));
        setMapel(updated);
      } else {
        const newId = `M-${Date.now()}`;
        const res = await createMataPelajaran(form);
        if (res && res.success === false) {
          setFormError(res.message || "Gagal membuat mata pelajaran.");
          setIsSaving(false);
          return;
        }
        const updated = [...mapel, { id_mapel: newId, ...form }];
        localStorage.setItem("data_mapel", JSON.stringify(updated));
        setMapel(updated);
      }
      mutate();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const updated = mapel.filter((m) => m.id_mapel !== id);
      localStorage.setItem("data_mapel", JSON.stringify(updated));
      setMapel(updated);
      mutate();
  ...
  const handleHapusSemua = async () => {
    if (confirm("Apakah Anda yakin ingin menghapus seluruh data mata pelajaran? Tindakan ini tidak dapat dibatalkan.")) {
      try {
        await deleteAllMataPelajaran();
        localStorage.setItem("data_mapel", JSON.stringify([]));
        setMapel([]);
        mutate();
        showToast("Seluruh data mata pelajaran berhasil dihapus!");
      } catch (err) {
        alert("Gagal menghapus seluruh data mata pelajaran.");
      }
    }
  };
      ...
      <main className="flex-1 md:ml-64 min-h-screen p-6 md:p-10 w-full transition-all pt-24 md:pt-10">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div className="flex items-center gap-3">
            <span className="material-symbols-outlined text-[#2563EB] text-3xl">book_2</span>
            <div>
              <h1 className="font-bold text-2xl text-slate-800">Mata Pelajaran</h1>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[720px]">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-[11px] uppercase tracking-widest text-slate-400 font-bold px-6 py-4 w-48">Kode Mapel</th>
                  <th className="text-[11px] uppercase tracking-widest text-slate-400 font-bold px-6 py-4">Nama Mata Pelajaran</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  filtered.map((m, idx) => (
                    <tr key={m.id_mapel} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-bold text-sm text-slate-700">{m.nama_mapel}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end items-center gap-1.5">
                          ...
                          <button
                            onClick={() => setDeleteConfirmId(m.id_mapel)}
          ...
          <div className="px-6 py-3 bg-slate-50 border-t border-slate-200">
            <p className="text-xs text-slate-400">
              Ditampilkan: {filtered.length} dari {mapel.length} data
            </p>
          </div>
        </div>
      </main>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <header className="flex items-center gap-3 mb-6">
              <span className="material-symbols-outlined text-[#2563EB] text-2xl">
                {editingMapel ? "edit_note" : "add_box"}
              </span>
            ...
            <div className="space-y-4">
              <div>
                <label className="font-bold text-slate-700 text-sm mb-1.5 block">Kode Mapel*</label>
                <input
                  type="text"
                  maxLength={6}
                  placeholder="MTK"
                  ...
                  onChange={(e) => setForm(p => ({ ...p, kode_mapel: e.target.value.toUpperCase() }))}
                />
              </div>

              <div>
                <label className="font-bold text-slate-700 text-sm mb-1.5 block">Nama Mata Pelajaran*</label>
                <input
                  type="text"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Hapus Mata Pelajaran</h3>
                <p className="text-slate-500 text-xs font-medium mt-1 leading-relaxed">
                  Apakah Anda yakin ingin menghapus data mata pelajaran ini? Tindakan ini akan menghapus data tersebut secara permanen.
                </p>
              </div>
            </div>
